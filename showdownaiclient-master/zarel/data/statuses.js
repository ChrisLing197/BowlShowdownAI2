...
exports.BattleStatuses = {
	...
	slp: {
		...
		onBeforeMove: function (pokemon, target, move) {
            ...
            pokemon.statusData.time++;
            // SCOTT: Reframing this for monte carlo rollout purposes.  Should be statistically equivalent
			if (pokemon.statusData.time > 1 && (pokemon.statusData.time >= 4 || this.random() <= 1.0 / (5 - pokemon.statusData.time))) {
				pokemon.cureStatus();
				return;
			}
	...
	confusion: {
		...
		onBeforeMove: function (pokemon) {
			...
			if (pokemon.statusData.time > 1 && (pokemon.statusData.time >= 5 || this.random() <= 1.0 / (6 - pokemon.statusData.time))) {
	...
	choicelock: {
		onStart: function (pokemon) {
			if (!this.activeMove || !this.activeMove.id || this.activeMove.sourceEffect && this.activeMove.sourceEffect !== this.activeMove.id) return false;
